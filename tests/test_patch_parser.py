...
from tests.helpers import build_fix_patch, build_new_source_patch, build_new_test_patch, create_phase2_repo


def test_patch_parser_accepts_safe_patch(tmp_path: Path) -> None:
    repo_root, _ = create_phase2_repo(tmp_path)
    result = validate_patch_text(build_fix_patch(), repo_root=repo_root, config=Config())
...
def test_patch_parser_allows_new_test_files(tmp_path: Path) -> None:
    repo_root, _ = create_phase2_repo(tmp_path)
    result = validate_patch_text(build_new_test_patch(), repo_root=repo_root, config=Config())
    assert result.valid is True
...
def test_patch_parser_blocks_new_source_files_by_default(tmp_path: Path) -> None:
    repo_root, _ = create_phase2_repo(tmp_path)
    result = validate_patch_text(build_new_source_patch(), repo_root=repo_root, config=Config())
    assert result.valid is False
