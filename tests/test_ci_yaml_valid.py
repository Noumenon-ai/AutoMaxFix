...
def test_ci_workflow_and_action_yaml_are_valid() -> None:
    workflow = yaml.safe_load(
        (ROOT / ".github" / "workflows" / "automaxfix-on-fail.yml").read_text()
    )
    action = yaml.safe_load(
        (ROOT / ".github" / "actions" / "automaxfix-action" / "action.yml").read_text()
    )
    ...
    assert any(
        step.get("uses") == "./.github/actions/automaxfix-action"
