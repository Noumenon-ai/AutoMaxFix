...
def _scan_spec(text: str, repo_root: Path) -> list[FailureRecord]:
    ...
    for index, match in enumerate(matches):
        block_end = (
            matches[index + 1].start() if index + 1 < len(matches) else len(detail_text)
        )
        ...
        while cursor < len(lines):
            ...
            if stripped.startswith(
                ("at ", "Assertion", "Error", "TypeError", "ReferenceError")
            ):
                break
        ...
        if location_file is None or line is None:
            continue
        summary = (
            first_summary_line(lines[cursor:], skip_prefixes=("at ",)) or "Test failed."
        )
        records.append(
            FailureRecord(
...
def _scan_tap(text: str, repo_root: Path) -> list[FailureRecord]:
    ...
    for index, match in enumerate(matches):
        ...
        location_file, line = (
            extract_location(at_match.group(1), repo_root) if at_match else (None, None)
        )
